...
pub fn usage_weekly_segment() -> SegmentConfig {
    // Own color, so the 7-day window reads apart from the 5-hour usage next to it
    let base = usage_segment();
    SegmentConfig {
        ...
        colors: ColorConfig {
            icon: Some(AnsiColor::Color256 { c256: 215 }), // Orange
            text: Some(AnsiColor::Color256 { c256: 215 }),
            background: None,
        },
        styles: base.styles,
        options: HashMap::new(),
